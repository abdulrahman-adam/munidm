          ...
          <table className="table-auto w-full">
            <thead className="text-gray-900 text-sm text-left bg-gray-50">
              <tr>
                <th className="px-2 py-2 md:px-4 md:py-3 font-semibold truncate">Product</th>
                <th className="px-2 py-2 md:px-4 md:py-3 font-semibold truncate">Category</th>
                {/* --- Added Variants Header --- */}
                <th className="px-2 py-2 md:px-4 md:py-3 font-semibold truncate hidden sm:table-cell">Variants</th>
                <th className="px-2 py-2 md:px-4 md:py-3 font-semibold truncate hidden md:table-cell">Price</th>
                <th className="px-2 py-2 md:px-4 md:py-3 font-semibold truncate">In Stock</th>
                <th className="px-2 py-2 md:px-4 md:py-3 font-semibold truncate">Action</th>
              </tr>
            </thead>
            ...
            <tbody className="text-sm text-gray-600">
              {products.map((product) => (
                <tr key={product.id} className="border-t border-gray-200 hover:bg-gray-50">
                  <td className="px-2 py-2 md:px-4 md:py-3 flex items-center space-x-2 md:space-x-3">
                    <div className="border border-gray-200 rounded overflow-hidden w-12 h-12 md:w-16 md:h-16 flex-shrink-0">
                      <img src={product.image[0]} alt="Product" className="w-full h-full object-contain" />
                    </div>
                    <span className="truncate max-w-[100px] md:max-w-none font-medium text-gray-800">{product.name}</span>
                  </td>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {product.variants && product.variants.length > 0 ? (
                        product.variants.map((variant, index) => (
                          <span key={index} className="px-2 py-0.5 bg-blue-50 text-blue-600 border border-blue-100 rounded text-[10px] md:text-xs">
                            {variant}
                          </span>
                        ))
                      ) : (
                  ...
                  <td className="px-2 py-2 md:px-4 md:py-3 text-center">
                    <button
                      className="flex justify-center items-center p-1 md:p-2 rounded hover:bg-red-100 mx-auto"
                      onClick={() => handleDelete(product.id)}
                    >
                      <img
                        src={assets.remove_icon}
